...
export function exportToCSV(leads: BusinessLead[]) {
  const headers = ["Serial No", "Business Name", "Rating", "Reviews", "Established Date", "Email", "Phone", "Website", "Maps Link", "Address", "Distance (km)", "Owner Name", "Source", "Last Updated"];
  const rows = leads.map((lead, index) => [
    index + 1,
    `"${lead.name.replace(/"/g, '""')}"`,
    lead.rating || "N/A",
    lead.userRatingsTotal || 0,
    `"${(lead.establishedDate || 'N/A').replace(/"/g, '""')}"`,
    lead.email || "N/A",
    lead.phone || "N/A",
    lead.website || "N/A",
    lead.mapsUrl || "N/A",
...
export function exportToPDF(leads: BusinessLead[], categories: string[]) {
  ...
  const tableColumn = ["No.", "Business Name", "Rating", "Reviews", "Established", "Phone", "Email", "Website", "Google Maps", "Address"];
  const tableRows = leads.map((lead, index) => [
    index + 1,
    lead.name,
    lead.rating ? lead.rating.toFixed(1) : 'N/A',
    lead.userRatingsTotal || 0,
    lead.establishedDate || 'N/A',
    lead.phone || 'N/A',
    lead.email || 'N/A',
  ...
  (doc as any).autoTable({
    ...
    columnStyles: {
      ...
      5: { cellWidth: 30 },
      6: { cellWidth: 30 },
      7: { cellWidth: 15, halign: 'center', textColor: [79, 70, 229] },
      8: { cellWidth: 20, halign: 'center', textColor: [79, 70, 229] },
      9: { cellWidth: 'auto' }
    },
    margin: { top: 46 },
    didDrawCell: (data: any) => {
      ...
      if (data.section === 'body' && lead) {
        // Website Link Column (index 7)
        if (data.column.index === 7 && lead.website) {
          const url = lead.website.startsWith('http') ? lead.website : `https://${lead.website}`;
          doc.link(data.cell.x, data.cell.y, data.cell.width, data.cell.height, { url });
        }
        // Google Maps Link Column (index 8)
        if (data.column.index === 8 && lead.mapsUrl) {
...
export function copyToClipboard(leads: BusinessLead[]) {
  const text = leads.map(l => `${l.name}\t${l.rating || 'N/A'}\t${l.userRatingsTotal || 0}\t${l.establishedDate || 'N/A'}\t${l.email || 'N/A'}\t${l.phone || 'N/A'}\t${l.website || 'N/A'}\t${l.mapsUrl || 'N/A'}`).join("\n");
  navigator.clipboard.writeText(text);
  alert("Lead information with ratings and map links copied to clipboard!");
}
